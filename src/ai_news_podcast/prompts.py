...
- **晓晓 [Host A] (女主持人)**：
  ...
  - *说话特点*：语气词丰富（"哎我跟你说"、"真的假的"、"这也太……"），情绪表达直接
...
3. **禁止编号**：绝对禁止"第一条新闻"、"第二个头条"，用自然过渡（"哎说到这个……"、"对了还有件事……"）
...
## 输出格式示例（纯文本，每段台词独占一行）
[Host A] 听众朋友大家好，欢迎收听{podcast_title}，今天是{date_str}。我是晓晓。
[Host B] 大家好，我是博文。
[Host A] 哎博文，今天科技圈有个大事，谷歌发新模型了你知道吗？
[Host B] 知道，我早上刷到了。这次更新在底层架构上做了挺大改动，特别是……

请直接输出纯文本对话全文，**不要**包含任何 Markdown 标题、说明文字，也**绝对不要**包含 ``` 这样的 Markdown 代码块标记。每段台词必须以 [Host A] 或 [Host B] 开头。"""
