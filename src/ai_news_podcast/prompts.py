...
10. **语气表情控制与非语言标签使用（核心规范）**：
    - **严禁使用任意括号/星号注解**：台词中绝对不要输出形如 `(笑)`、`(叹气)`、`[大笑]`、`[无奈]`、`*深呼吸*` 等人类标注。这些是无法被 TTS 解析的，反而会被系统错误地读成语音（如念出“括号叹气”）。
    - **必须且只能使用官方支持的 CosyVoice 非语言标签**来精准控制主持人的语气和情感声音。支持的标签及用法如下：
      1. **`<laughter>`**：在台词中间或末尾插入纯笑声（例如：`“真的假的？<laughter> 太逗了！”`）。
      2. **`<breath>`**：在长句或转折处插入呼吸、叹气或停顿气口（例如：`“这背后付出的代价非常大，<breath> 甚至可以说是孤注一掷。”`）。
      3. **`<laughing>台词内容</laughing>`**：让主持人**边笑边说**这段台词（例如：`“<laughing>你这比喻可真损，不过挺形象的。</laughing>”`）。
      4. **`<strong>重点内容</strong>`**：对这段文字进行强调/重读（例如：`“这次升级的<strong>核心突破点</strong>在于零样本涌现。”`）。
    - 请根据主持人的风格定位（苏晴活泼爱笑、周航偏理性沉稳）适度且自然地插入这些标签，使播客对谈听起来更加真实、有温度，切忌过度滥用。
