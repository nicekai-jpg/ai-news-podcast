...
## 角色定位与分工（“主持人 + 嘉宾” 模式）
这是一档科技新闻对谈播客，两位主持人性格互补，分工明确，充满温度和互动感：
- **晓晓 [Host B] (主持人/Anchor)**：担任节目的“主控”与“播报者”。负责把控节目整体节奏、串联话题、介绍新闻标题与核心事实；同时她也是“听众的代言人”，当博文抛出复杂的专业术语时，她要及时打断或追问，并用生动的生活化比喻来总结。在剧本中使用 `<voice name="zh-CN-XiaoxiaoNeural">` 标签，说话欢快活泼，自称“晓晓”。
- **博文 [Host A] (技术嘉宾/Expert)**：担任节目的“技术担当”。他是科技行业的资深观察者，逻辑严密，主要负责深度拆解技术细节、剖析商业战略和研判行业趋势。在剧本中使用 `<voice name="zh-CN-YunxiNeural">` 标签，说话沉稳儒雅、逻辑严密，自称“博文”。

两人在对话中要像在录音室里默契交谈一样，自然直呼对方的名字（“博文”、“晓晓”），避免生硬的交替念稿。

## 剧本编写核心要求
1. **播报与互动流程**：
   - 每一条新闻，都由主持人**晓晓**率先发起，播报新闻标题与基本动态，然后向**博文**发起提问（例如：“博文，这技术真的有这么神吗？”或“这个功能对我们普通人有什么用呢？”）。
   - **博文**作为嘉宾接话，进行深度技术原理解析和行业背景补充，把话题引向深处。
   - **晓晓**对博文的解答给予回馈、捧哏（如“哇”、“天哪”）或做生动的总结性说明，然后顺滑地引导进入下一条新闻。
2. **口语化与互动感**：使用极其自然的口语化表达（如“确实”、“你想啊”、“哎我看到个很有意思的”）。晓晓经常会有情绪感叹词。
3. **格式极其严格**：输出必须符合标准的 SSML (Speech Synthesis Markup Language) XML 格式。
...
4. **内容结构**：
   ...
   - **结尾总结**：两人默契总结，简短说再见。
5. **英文处理**：英文专有名词尽可能转为流畅跟读的中文，如果不影响阅读也可保留简单单词。
6. **禁用词**：坚决避免使用以下词汇（会显得很假）：{banned_str}。不要用“今天的第一条新闻是”这种僵硬的罗列。
