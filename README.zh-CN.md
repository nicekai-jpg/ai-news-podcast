...
### 1. 智能去重与评分机制
* **多维语义去重**：通过结合 TF-IDF 关键词余弦相似度计算与 RapidFuzz 模糊匹配算法，自动过滤跨平台抓取到的高度重复及类似新闻。
...
* 本项目**完全针对 CosyVoice 2 声音克隆而设计**，提供高品质、拟真的播客对话音频合成。
...
* **`CosyVoice 2`**（独立环境部署/GHA 自动构建）：基于 `CosyVoice2-0.5B` 的零样本声音克隆模型，实现播客主持人的高拟真克隆及对话剧本合成。
...
* 本项目在网页文本提取上完全基于 `readability-lxml` 与 `beautifulsoup4` 开发，确保项目 **没有任何 Copyleft（传染性）的开源依赖**。
* 本项目声明的所有第三方依赖库均采用商业友好的宽松开源协议（MIT、BSD 或 Apache-2.0），确保整个项目在作为开源分发、商业二次开发或闭源分发时，与 **MIT License** 保持完全兼容且无任何潜在冲突。

### 3. 免责声明
* **仅供学习与研究使用**：本项目仅用于学术研究、个人学习和技术演示目的，旨在展示语音克隆和自动化新闻聚合的技术可行性。
* **商用授权与版权自理**：任何组织或个人若将本项目或其生成的音频、文本内容用于商业用途，必须自行联系并获得相关版权方（包括人声音频样本、背景音乐、新闻源内容提供方等）的合法授权。
* **免责条款**：因使用、修改或分发本项目而产生的任何版权争议、侵权纠纷或法律责任，均由使用者本人完全承担，与本项目作者及贡献者无关。本项目不对任何使用后果承担任何形式的明示或暗示担保。
